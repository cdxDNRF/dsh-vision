...
export function apply(ctx, config) {
  ...
  // 0.1.0-rc.6 的 Web 设置客户端只暴露两类 settings 命名空间：apiproxy
  // 内置白名单与「可配置 LLM provider」目录。视觉网关是 OpenAI 兼容的 LLM
  // 端点，按官方契约声明为可配置 provider 即可让「插件配置」GUI 卡片读到
  // dsh-vision 命名空间（否则卡片永远 unavailable 而隐藏）。
  // 故意不注册 adapter：视觉服务不是 agent 的 LLM 路由，不会进入模型目录
  // 与请求分发；即便用户在模型页添加该路由，schema 校验也会拒绝不认识的字段。
