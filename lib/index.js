...
const FIELDS = [
  ...
  { key: 'maxTokens', type: 'number', step: 1, min: 1, default: 65536 },
...
export function apply(ctx, config) {
  ...
  // 此工具专为文本模型设计：对本地文件走外部视觉 API 做文字描述后返回。
  // 若当前模型本身支持图片输入（多模态模型），agent 应优先使用原生
  // read_image 工具直接读取图片像素，无需调用本工具。
  const disposeTool = ctx.tools.register({
    ...
    description:
      ...
      + 'NOTE: If the current model already supports image input (multimodal), prefer the native `read_image` tool instead — '
      + 'it passes the raw pixels to the model and avoids the external vision API call.',
    parameters: {
      type: 'object',
      ...
      properties: {
        source: {
          ...
          description: 'Local image file path (absolute or workspace-relative) or an http(s):// image URL. '
            + 'For local files on a multimodal model, use `read_image` instead.',
        },
        question: {
          type: 'string',
