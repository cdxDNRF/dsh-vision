...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // 官方 Slot 注入写法：`settings.plugin.item` 由 settings-plugins 客户端插件
      // 声明，本插件与其加载顺序不定；直接 slots.register 在声明提交前会抛
      // "slot is not declared" 并使整个前端进入 Failed to load plugins 白屏。
      // slots.inject 会在声明存在时同步运行回调，否则等声明提交后再注册
      // （与官方 settings-plugins 的卡片注册、wishadel 皮肤同款写法）。
