 ...
 *  - 注册进 `settings.plugin.item` 键控 Slot，`key` 必须等于宿主设置命名空间
 *    `dsh-vision`（dsh 0.1.0-rc.7 起该 Slot 不再接受 `id`）；
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      // dsh 0.1.0-rc.7 将该 Slot 从 list 改为 keyed：key 必须是与 Host
      // settings.register() / settings.describe() 返回值一致的命名空间，否则
      // keyed slot 会在注册时抛错，或因未被 ConfigurablePluginsTab 派发而静默消失。
