...
/** 取模型信息的真实输入模态；信息非法时返回 undefined（视为未知，不主动桥接）。 */
function modalitiesOf(info) {
  return info === null || typeof info !== 'object' ? undefined : info.inputModalities
}

/** 给模型信息补上 image 能力（幂等；插件关闭时不动）。 */
function withImageModality(info, deps) {
  if (!deps.enabled()) return info
  const mods = modalitiesOf(info)
...
/**
 * 图片→文字桥接的流包装：`isTextOnly(options)` 判定为真时把图片块
 * 递归替换成视觉模型描述再交给适配器；多模态模型原样直传（零开销）。
 */
async function* streamWithBridge(origStream, options, state, deps, isTextOnly) {
  if (deps.enabled() && messagesHaveImage(options.messages)) {
    try {
      if (await isTextOnly(options)) {
...
/**
 * 包装一个适配器实例，覆盖两代契约；重复调用跳过（同一实例只包装一次）。
 *
 * - 旧契约（≤ dsh 0.1.2）：适配器暴露 `resolveModel` + `stream`；
 * - 新契约（dsh 0.1.5+）：适配器用 `prepareCall(provider, model, signal)`
 *   一次性绑定 `{ model, stream }`，运行时改用 `prepareCall().stream` 派发，
 *   `resolveModel`/`stream` 上的包装会被完全绕过——而且 `dsh-llm` 会在
 *   **适配器之前**检查 `prepareCall().model.inputModalities`，不含 image 时
 *   直接把图片替换成 `[image omitted because this model accepts text only…]`
 *   占位文本。因此新契约下必须包装 `prepareCall`，否则桥接完全失效。
 */
function wrapAdapter(runtime, providerId, deps, wrapped) {
  ...
  // --- 旧契约：resolveModel（新版本中 resolveModelInfo 仍会调用它）---
  const origResolveModel = typeof adapter.resolveModel === 'function' ? adapter.resolveModel.bind(adapter) : undefined
  const wrappedResolveModel = origResolveModel === undefined
    ? undefined
    : async function resolveModel(provider, model, signal) {
        return withImageModality(await origResolveModel(provider, model, signal), deps)
      }

  // --- 旧契约：stream ---
  const origStream = typeof adapter.stream === 'function' ? adapter.stream.bind(adapter) : undefined
  const isTextOnlyByResolve = async (options) => {
    if (origResolveModel === undefined) return false
    const mods = modalitiesOf(await origResolveModel(options.provider, options.model, options.signal))
    return mods !== undefined && !mods.includes('image')
  }
  const wrappedStream = origStream === undefined
    ? undefined
    : function stream(options) {
        return streamWithBridge(origStream, options, state, deps, isTextOnlyByResolve)
      }

  // --- 新契约：prepareCall（0.1.5+ 的实际派发路径）---
  const hadPrepareCall = typeof adapter.prepareCall === 'function'
  const origPrepareCall = hadPrepareCall ? adapter.prepareCall.bind(adapter) : undefined
  const wrappedPrepareCall = hadPrepareCall
    ? async function prepareCall(provider, model, signal) {
        const call = await origPrepareCall(provider, model, signal)
        if (call === null || typeof call !== 'object') return call
        const mods = modalitiesOf(call.model)
        // 能力未知或模型本身支持图片：保持原样
        if (!deps.enabled() || mods === undefined || mods.includes('image')) return call
        const inner = typeof call.stream === 'function' ? call.stream.bind(call) : origStream
        if (inner === undefined) return call
        return {
          ...call,
          // 声明 image 能力：阻止 dsh-llm 把图片投影成占位文本，交给下面的桥接处理
          model: { ...call.model, inputModalities: [...mods, 'image'] },
          stream: (options) => streamWithBridge(inner, options, state, deps, async () => true),
        }
      }
    : undefined

  if (wrappedResolveModel !== undefined) adapter.resolveModel = wrappedResolveModel
  if (wrappedStream !== undefined) adapter.stream = wrappedStream
  if (wrappedPrepareCall !== undefined) adapter.prepareCall = wrappedPrepareCall
  wrapped.set(adapter, { origResolveModel, origStream, origPrepareCall, hadPrepareCall })
}

function unwrapAll(wrapped) {
  for (const [adapter, originals] of wrapped) {
    try {
      if (originals.origResolveModel !== undefined) adapter.resolveModel = originals.origResolveModel
      if (originals.origStream !== undefined) adapter.stream = originals.origStream
      if (originals.hadPrepareCall === true) adapter.prepareCall = originals.origPrepareCall
    } catch { /* 适配器可能已被替换/冻结 */ }
  }
  wrapped.clear()
}
